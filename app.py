...
@app.route('/predict', methods=['GET', 'POST'])  # Fixed: 'post' → 'POST'
def predict():
    try:
        ...
        prediction = model.predict([[
            Operating_Gross_Margin, Cash_flow_rate, Tax_rate_A,
            Net_worth_Assets, Total_Asset_Turnover, Cash_Total_Assets,
            Cash_Current_Liability, Total_income_Total_expense,
            Total_expense_Assets, Gross_Profit_to_Sales
        ]])

        output = prediction[0]
        result_text = "⚠️ Bankrupt (High Risk)" if output == 1 else "✅ Not Bankrupt (Low Risk)"

        return render_template('indexs.html', result=result_text)

    except Exception as e:
        return render_template('indexs.html', result=f"Error: {str(e)}")
